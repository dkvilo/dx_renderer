...
#if !defined( HANDMADE_MATH_USE_DEGREES ) && !defined( HANDMADE_MATH_USE_TURNS ) &&                          \
...
#define HMM_SQUARE( x ) ( ( x ) * ( x ) )
	...
	typedef union HMM_Mat2
	{
		...
		inline const HMM_Vec2 &operator[]( int Index ) const
		{
			return Columns[Index];
		}
#endif
	...
	typedef union HMM_Mat3
	{
		...
		inline const HMM_Vec3 &operator[]( int Index ) const
		{
			return Columns[Index];
		}
#endif
	...
	typedef union HMM_Mat4
	{
		...
		inline const HMM_Vec4 &operator[]( int Index ) const
		{
			return Columns[Index];
		}
#endif
...
#else
	Result =
	    ( ( Left.X * Right.X ) + ( Left.Z * Right.Z ) ) + ( ( Left.Y * Right.Y ) + ( Left.W * Right.W ) );
#endif
 ...
 */
	...
	static inline HMM_Vec4 HMM_LinearCombineV4M4( HMM_Vec4 Left, HMM_Mat4 Right )
	{
		...
		Result.SSE =
		    _mm_add_ps( Result.SSE,
		                _mm_mul_ps( _mm_shuffle_ps( Left.SSE, Left.SSE, 0x55 ), Right.Columns[1].SSE ) );
		Result.SSE =
		    _mm_add_ps( Result.SSE,
		                _mm_mul_ps( _mm_shuffle_ps( Left.SSE, Left.SSE, 0xaa ), Right.Columns[2].SSE ) );
		Result.SSE =
		    _mm_add_ps( Result.SSE,
		                _mm_mul_ps( _mm_shuffle_ps( Left.SSE, Left.SSE, 0xff ), Right.Columns[3].SSE ) );
#elif defined( HANDMADE_MATH__USE_NEON )
 ...
 */
	...
	static inline float HMM_DeterminantM4( HMM_Mat4 Matrix )
	{
		...
		HMM_Vec3 B10 = HMM_SubV3( HMM_MulV3F( Matrix.Columns[0].XYZ, Matrix.Columns[1].W ),
		                          HMM_MulV3F( Matrix.Columns[1].XYZ, Matrix.Columns[0].W ) );
		HMM_Vec3 B32 = HMM_SubV3( HMM_MulV3F( Matrix.Columns[2].XYZ, Matrix.Columns[3].W ),
	...
	static inline HMM_Mat4 HMM_InvGeneralM4( HMM_Mat4 Matrix )
	{
		...
		HMM_Mat4 Result;
		Result.Columns[0] = HMM_V4V(
		    HMM_AddV3( HMM_Cross( Matrix.Columns[1].XYZ, B32 ), HMM_MulV3F( C23, Matrix.Columns[1].W ) ),
		    -HMM_DotV3( Matrix.Columns[1].XYZ, C23 ) );
		Result.Columns[1] = HMM_V4V(
		    HMM_SubV3( HMM_Cross( B32, Matrix.Columns[0].XYZ ), HMM_MulV3F( C23, Matrix.Columns[0].W ) ),
		    +HMM_DotV3( Matrix.Columns[0].XYZ, C23 ) );
		Result.Columns[2] = HMM_V4V(
		    HMM_AddV3( HMM_Cross( Matrix.Columns[3].XYZ, B10 ), HMM_MulV3F( C01, Matrix.Columns[3].W ) ),
		    -HMM_DotV3( Matrix.Columns[3].XYZ, C01 ) );
		Result.Columns[3] = HMM_V4V(
		    HMM_SubV3( HMM_Cross( B10, Matrix.Columns[2].XYZ ), HMM_MulV3F( C01, Matrix.Columns[2].W ) ),
 ...
 */
	...
	static inline HMM_Mat4
	HMM_Orthographic_RH_NO( float Left, float Right, float Bottom, float Top, float Near, float Far )
	{
	...
	// Near and Far specify the distances to the near and far clipping planes.
	static inline HMM_Mat4
	HMM_Orthographic_RH_ZO( float Left, float Right, float Bottom, float Top, float Near, float Far )
	{
	...
	// Near and Far specify the distances to the near and far clipping planes.
	static inline HMM_Mat4
	HMM_Orthographic_LH_NO( float Left, float Right, float Bottom, float Top, float Near, float Far )
	{
	...
	// Near and Far specify the distances to the near and far clipping planes.
	static inline HMM_Mat4
	HMM_Orthographic_LH_ZO( float Left, float Right, float Bottom, float Top, float Near, float Far )
	{
 ...
 */
...
#ifdef HANDMADE_MATH__USE_SSE
		__m128 SSEResultOne   = _mm_xor_ps( _mm_shuffle_ps( Left.SSE, Left.SSE, _MM_SHUFFLE( 0, 0, 0, 0 ) ),
                                          _mm_setr_ps( 0.f, -0.f, 0.f, -0.f ) );
		__m128 SSEResultTwo   = _mm_shuffle_ps( Right.SSE, Right.SSE, _MM_SHUFFLE( 0, 1, 2, 3 ) );
		__m128 SSEResultThree = _mm_mul_ps( SSEResultTwo, SSEResultOne );

		SSEResultOne   = _mm_xor_ps( _mm_shuffle_ps( Left.SSE, Left.SSE, _MM_SHUFFLE( 1, 1, 1, 1 ) ),
                                   _mm_setr_ps( 0.f, 0.f, -0.f, -0.f ) );
		SSEResultTwo   = _mm_shuffle_ps( Right.SSE, Right.SSE, _MM_SHUFFLE( 1, 0, 3, 2 ) );
		SSEResultThree = _mm_add_ps( SSEResultThree, _mm_mul_ps( SSEResultTwo, SSEResultOne ) );

		SSEResultOne   = _mm_xor_ps( _mm_shuffle_ps( Left.SSE, Left.SSE, _MM_SHUFFLE( 2, 2, 2, 2 ) ),
                                   _mm_setr_ps( -0.f, 0.f, 0.f, -0.f ) );
	...
	Result.NEON =
	    vfmaq_f32( Result.NEON, Right2301, vmulq_f32( vdupq_laneq_f32( Left.NEON, 1 ), SecondSign ) );
	float32x4_t ThirdSign = { -1.0f, 1.0f, 1.0f, -1.0f };
	Result.NEON =
	    vfmaq_f32( Result.NEON, Right1032, vmulq_f32( vdupq_laneq_f32( Left.NEON, 2 ), ThirdSign ) );
	...
	static inline HMM_Quat HMM_M4ToQ_RH( HMM_Mat4 M )
	{
		...
		if ( M.Elements[2][2] < 0.0f )
		{
			...
			else
			{
				...
				T = 1 - M.Elements[0][0] + M.Elements[1][1] - M.Elements[2][2];
				Q = HMM_Q( M.Elements[0][1] + M.Elements[1][0],
				           T,
				           M.Elements[1][2] + M.Elements[2][1],
				           M.Elements[2][0] - M.Elements[0][2] );
			}
...
#define HMM_Len( A ) _Generic( ( A ), HMM_Vec2: HMM_LenV2, HMM_Vec3: HMM_LenV3, HMM_Vec4: HMM_LenV4 )( A )

#define HMM_LenSqr( A )                                                                                      \
	_Generic( ( A ), HMM_Vec2: HMM_LenSqrV2, HMM_Vec3: HMM_LenSqrV3, HMM_Vec4: HMM_LenSqrV4 )( A )
...
#define HMM_Norm( A )                                                                                        \
	_Generic( ( A ),                                                                                         \
	    ...
	    HMM_Vec3: HMM_NormV3,                                                                                \
	    HMM_Vec4: HMM_NormV4,                                                                                \
	    HMM_Quat: HMM_NormQ )( A )

#define HMM_Dot( A, B )                                                                                      \
	_Generic( ( A ), HMM_Vec2: HMM_DotV2, HMM_Vec3: HMM_DotV3, HMM_Vec4: HMM_DotV4, HMM_Quat: HMM_DotQ )(    \
	    A,                                                                                                   \
	    B )

#define HMM_Lerp( A, T, B )                                                                                  \
	_Generic( ( A ), float: HMM_Lerp, HMM_Vec2: HMM_LerpV2, HMM_Vec3: HMM_LerpV3, HMM_Vec4: HMM_LerpV4 )(    \
	    A,                                                                                                   \
	    T,                                                                                                   \
	    B )

#define HMM_Eq( A, B ) _Generic( ( A ), HMM_Vec2: HMM_EqV2, HMM_Vec3: HMM_EqV3, HMM_Vec4: HMM_EqV4 )( A, B )

#define HMM_Transpose( M )                                                                                   \
	_Generic( ( M ), HMM_Mat2: HMM_TransposeM2, HMM_Mat3: HMM_TransposeM3, HMM_Mat4: HMM_TransposeM4 )( M )
...
#define HMM_InvGeneral( M )                                                                                  \
	_Generic( ( M ), HMM_Mat2: HMM_InvGeneralM2, HMM_Mat3: HMM_InvGeneralM3, HMM_Mat4: HMM_InvGeneralM4 )( M )
